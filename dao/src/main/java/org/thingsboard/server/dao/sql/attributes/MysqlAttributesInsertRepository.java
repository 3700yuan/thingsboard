...
public class MysqlAttributesInsertRepository extends AttributeKvInsertRepository {
    ...
    private static final String INSERT_OR_UPDATE = "insert into attribute_kv(entity_type, entity_id, attribute_type, attribute_key, str_v, long_v, dbl_v, bool_v, json_v, last_update_ts) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) on duplicate key update entity_type=?,entity_id=?,attribute_type=?,attribute_key=?";
    ...
    protected void saveOrUpdate(List<AttributeKvEntity> entities) {
        entities.forEach(entity -> {
            jdbcTemplate.update(INSERT_OR_UPDATE, ps -> {
                ...
                ps.setString(11, entity.getId().getEntityType().name());
                ps.setString(12, entity.getId().getEntityId());
                ps.setString(13, entity.getId().getAttributeType());
                ps.setString(14, entity.getId().getAttributeKey());
            });
        });
    }
}