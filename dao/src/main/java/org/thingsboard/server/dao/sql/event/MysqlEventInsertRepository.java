...
public class MysqlEventInsertRepository extends AbstractEventInsertRepository {
    ...
    private static final String P_KEY_CONFLICT_STATEMENT = "event.id=:id";
    private static final String UNQ_KEY_CONFLICT_STATEMENT = "event.tenant_id=:tenant_id AND event.entity_type=:entity_type AND event.entity_id=:entity_id AND event.event_type=:event_type AND event.event_uid=:event_uid";
    ...
    private static String getInsertString(String conflictStatement) {
        ...
        */
        log.info("conflictStatement = "+conflictStatement);
        String sql = "insert into event(id, body, entity_id, entity_type, event_type, event_uid, tenant_id, ts) VALUES (:id, :body, :entity_id, :entity_type, :event_type, :event_uid, :tenant_id, :ts) on duplicate key update "+conflictStatement;
        log.info("sql = "+sql);
        return sql;
